 ...
 *  Problem Set 1: Echo Protocol (Client)
...
#define MESSAGE "somE defAULt messAGE"
...
int main (int argc, char *argv[]) {
    ...
    // message to send. user defined or revert to the default MESSAGE
    char * user_message = MESSAGE;
    ...
    if ( send( socket_file_descriptor, user_message, strlen(user_message), 0) < 0){
        close( socket_file_descriptor );
        error("Failed to send message\n");
    }

    // process response
    // zero out the buffer so there's no garbage in the memory, especially if the message is too short
    bzero(buffer, BUFFER_SIZE);
    if ( read(socket_file_descriptor, buffer, BUFFER_SIZE) < 0 ){
        close(socket_file_descriptor);
        error("Client could not read response from the server. Exiting...\n");
    } else {
